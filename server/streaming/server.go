...
type Server struct {
	...
	ackChan chan (*telemetry.Record) `json:"-"`
...
func InitServer(c *config.Config, airbrakeHandler *airbrake.AirbrakeHandler, producerRules map[string][]telemetry.Producer, logger *logrus.Logger, registry *SocketRegistry) (*http.Server, *Server, error) {
	...
	logger.ActivityLog("InitServer", logrus.LogInfo{"SocketServer": socketServer.DispatchRules})
	...
	logger.ActivityLog("MUX", logrus.LogInfo{"mux": mux})
	logger.ActivityLog("C Ackchan", logrus.LogInfo{"ackChan": c.AckChan})
	logger.ActivityLog("C ReliableAcksources", logrus.LogInfo{"reliableAcksources": c.ReliableAckSources})
	logger.ActivityLog("AirbrakeHandler", logrus.LogInfo{"airbrake": airbrakeHandler})

	server := &http.Server{Addr: fmt.Sprintf("%v:%v", c.Host, c.Port), Handler: serveHTTPWithLogs(mux, logger)}
	logger.ActivityLog("Port", logrus.LogInfo{"server_port": c.Port})
	logger.ActivityLog("Host", logrus.LogInfo{"server_host": c.Host})
	logger.ActivityLog("Server", logrus.LogInfo{"server": server})
